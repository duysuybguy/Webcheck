...
class AdminController extends Controller
{
    ...
    public function chapterManagerment(){
        return view('admin.chapter-management');
    }
    ...
    public function getListChapter(Request $request){
        $chapters = Chapter::withCount('questions')->with('cate')->get();
        $res = [
            'rc'=>'0',
            'data'=> $chapters
        ];
    ...
    public function reloadChapter(Request $request){
        $req = $request->all();
        if (isset($req['chapterId'])){
            $chap = Chapter::find($req['chapterId']);
            if ($chap){
                try {
                    $log = Chapter::loadQuestionFromSource($chap);
                    return json_encode([
                        'rc'=>'0',
                        'data'=> $log,
                    ]);
    ...
    public function deleteChapterData(Request $request){
        $req = $request->all();
        if (isset($req['chapterId'])){
            Log::error('Check delete data');
            $chapterId = $req['chapterId'];
            DB::delete('delete from choice where questionId in (select questionId from question where chapterId = ?)', [$chapterId]);
            DB::delete('delete from question where chapterId = ?', [$chapterId]);
            return json_encode([
                'rc'=>'0',
