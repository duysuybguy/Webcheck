...
export default {
  ...
  methods:{
    ...
    resetData(){
        localStorage.removeItem('local_question_data');
    ...
    // Không tồn tại dữ liệu
      ...
      axios.get(`/questions/${this.chapterId}`)
        .then(res => {
          ...
          let tempData = res['data']['questions'];
          this.localQuestion.data = tempData.map(item => {
            if (item.question_type === 'RADIO' || item.question_type === 'radio'){
              let countChoice = item.choices.filter(choice => choice.isTrue === 1).length;
              if (countChoice > 1)
